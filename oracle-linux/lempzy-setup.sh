...
check_package_installed() {
    ...
    if command -v dpkg >/dev/null 2>&1; then
        dpkg -l | grep -q "^ii.*$package_name" && return 0 || return 1
    elif command -v rpm >/dev/null 2>&1; then
        rpm -q "$package_name" >/dev/null 2>&1 && return 0 || return 1
    elif command -v dnf >/dev/null 2>&1; then
        dnf list installed "$package_name" >/dev/null 2>&1 && return 0 || return 1
    elif command -v yum >/dev/null 2>&1; then
        yum list installed "$package_name" >/dev/null 2>&1 && return 0 || return 1
    else
        return 1
    fi
}
...
if [[ "$OS_ID" != "ol" ]]; then
    echo -e "${red}This oracle-linux script must be run on Oracle Linux (ol).${end}"
    exit 1
fi
...
# Installing Firewall
INSTALL_FIREWALL=scripts/install/install_firewall.sh

if test -f "$INSTALL_FIREWALL"; then
     source $INSTALL_FIREWALL
     if [ -d "$HOME/Lempzy" ]; then
          cd && cd Lempzy
     else
          echo "${yel}Lempzy directory not found, staying in current directory${end}"
     ...
     echo "${red}Cannot install firewall${end}"
     exit
fi
...
INSTALL_NODEJS=scripts/install/install_nodejs.sh
if check_command_exists "node"; then
    echo "${grn}Node.js is already installed, skipping...${end}"
else
    if test -f "$INSTALL_NODEJS"; then
        echo "${grn}Installing Node.js...${end}"
        if source "$INSTALL_NODEJS"; then
            echo "${grn}Node.js installed successfully${end}"
        else
            add_failed_installation "Node.js"
        fi
        cd && cd Lempzy
    else
        echo "${red}Cannot find Node.js installation script${end}"
        add_failed_installation "Node.js (script not found)"
    fi
fi

INSTALL_GOLANG=scripts/install/install_golang.sh
if check_command_exists "go"; then
    echo "${grn}Go is already installed, skipping...${end}"
else
    if test -f "$INSTALL_GOLANG"; then
        echo "${grn}Installing Go...${end}"
        if source "$INSTALL_GOLANG"; then
            echo "${grn}Go installed successfully${end}"
        else
            add_failed_installation "Go"
        fi
        cd && cd Lempzy
    else
        echo "${red}Cannot find Go installation script${end}"
        add_failed_installation "Go (script not found)"
    fi
fi
...
if check_command_exists "inotifywait"; then
    ...
    if command -v dnf >/dev/null 2>&1; then
        dnf install -y inotify-tools || dnf install -y inotify-tools-devel || true
    elif command -v yum >/dev/null 2>&1; then
        yum install -y inotify-tools || yum install -y inotify-tools-devel || true
    else
